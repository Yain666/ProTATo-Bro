...
public static class MainMenuSettingsRuntimeFactory
{
    public static MainMenuSettingsPanel GetOrCreate(Transform parent)
    {
        if (parent == null)
        {
            return null;
        ...
        MainMenuSettingsPanel existing = parent.GetComponentInChildren<MainMenuSettingsPanel>(true);
        if (existing != null)
        {
            return existing;
        }
        ...
        GameObject root = CreateUIObject("MainMenuSettingsPanel", parent, out RectTransform rootRect, true);
        Stretch(rootRect);
